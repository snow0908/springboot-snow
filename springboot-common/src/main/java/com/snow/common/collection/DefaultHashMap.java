package com.snow.common.collection;
 ...
 * <li>从hash表中取得的任何<code>Iterator</code>具有<i>fail-fast</i>特性: 当hash表的结构被改变时, 调用 <code>Iterator.remove</code>和
 * <code>Iterator.add</code>方法时, 会掷出 <code>ConcurrentModificationException</code>. 这样确保不会出现不确定的情况.</li>
 * </ul>
 *
...
public class DefaultHashMap<K, V> extends AbstractMap<K, V> implements Map<K, V>, Cloneable, Serializable {
    ...
    /**
     * 阈值. 当hash表中的entry数超过它时, 自动扩容(<code>resize</code>). 其值等于 <code>capacity&times;loadFactor</code>.
     *
     * @serial 自动序列化字段
     */
    ...
    /**
     * 当hash表发生&quot;结构改变&quot;的计数. 所谓&quot;结构改变&quot;, 是指hash表中entry的数目发生改变或内部结构改变(如<code>resize</code>).
    ...
    /**
     * 返回指定key对应的value. 如果hash表中没有value对应key, 则返回<code>null</code>. 但是返回 <code>null</code>并不总是代表没有value对应指定的key, 也有可能是指
     * value值本身为<code>null</code> . 可以通过方法<code>containsKey</code>来区分这两 种情况.
     *
     ...
     * @return 如果已经存在和此key相关联的value, 则返回此value. 否则返回<code>null</code>. 返回 <code>null</code>也可能是因为被取代的这个value值为
     *         <code>null</code>.
     */
    @Override
     ...
     * @return 被删除的entry的value. 如果entry不存在, 则返回<code>null</code>. 但是返回 <code>null</code>并不总是代表没有value对应指定的key, 也有可能是指
     *         value值本身为 <code>null</code>.
     */
    @Override
    ...
    /**
     * 取得key的集合视图. 这个集合是以hash表为基础的, 任何hash表的改变, 都会反射到这个集合, 反之亦然. 该集合支持删除操作, 删除集合中的key就删除了hash表中的相应entry.
     * 可以通过以下方法删除一个entry: <code>Iterator.remove</code>, <code>Set.remove</code>, <code>removeAll</code>,
     * <code>retainAll</code>, 和<code>clear</code>. 但集合不支持<code>add</code>或<code>addAll</code>操作.
     *
     * @return key的集合视图
     */
    @Override
    ...
    /**
     * 取得value的集合视图. 这个集合是以hash表为基础的, 任何hash表的改变, 都会反射到这个集合, 反之亦然. 该集合支持删除操作, 删除集合中的key就删除了hash表中的相应entry.
     * 可以通过以下方法删除一个entry: <code>Iterator.remove</code>, <code>Collection.remove</code>, <code>removeAll</code>,
     * <code>retainAll</code>, 和<code>clear</code>. 但集合不支持<code>add</code>或<code>addAll</code>操作.
     *
     * @return value的集合视图
     */
    ...
    /**
     * 取得entry的集合视图. 这个集合是以hash表为基础的, 任何hash表的改变, 都会反射到这个集合, 反之亦然. 该集合支持删除操作, 删除集合中的key就删除了hash表中的相应entry.
     * 可以通过以下方法删除一个entry: <code>Iterator.remove</code>, <code>Set.remove</code>, <code>removeAll</code>,
     * <code>retainAll</code>, 和<code>clear</code>. 但集合不支持<code>add</code>或<code>addAll</code>操作.
     *
     * @return entry的集合视图
    ...
    /**
     * 给子类一个机会初始化自己. 该方法被所有构造函数以及&quot;伪构造函数&quot;(<code>clone</code>, <code>readObject</code>)调用. 调用时, hash表已被初始化,
     * 但数据尚未被插入到表中.
     */
