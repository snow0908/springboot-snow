...
import com.snow.common.exception.ClassInstantiationException;
import com.snow.common.exception.ServiceNotFoundException;
import com.snow.common.io.StreamUtil;
 ...
 *
 * <p>
 * <code>ClassLoaderUtil</code>查找类和资源的效果， 相当于<code>ClassLoader.loadClass</code> 方法和<code>ClassLoader.getResource</code>
 * 方法。 但<code>ClassLoaderUtil</code> 总是首先尝试从<code>Thread.getContextClassLoader()</code>方法取得 <code>ClassLoader</code>
 ...
 * <li>
 * 类<code>B</code>是Web Application中的一个类，是由servlet引擎的<code>ClassLoader</code> 动态装入的</li>
 * <li>
 * 资源文件<code>C.properties</code>也在Web Application中，只有servlet引擎的动态 <code>ClassLoader</code>可以找到它</li>
 * <li>
 * 类<code>B</code>调用工具类<code>A</code>的方法，希望通过类<code>A</code>取得资源文件 <code>C.properties</code></li>
 * </ul>
 *
 * <p>
 * 如果类<code>A</code>使用 <code>getClass().getClassLoader().getResource(&quot;C.properties&quot;)</code> ， 就会失败，因为系统
 * <code>ClassLoader</code>不能找到此资源。 但类A可以使用ClassLoaderUtil.getResource(&quot;C.properties&quot;)，就可以找到这个资源，
 * 因为ClassLoaderUtil调用<code>Thread.currentThead().getContextClassLoader()</code> 取得了servlet引擎的<code>ClassLoader</code>，
 * 从而找到了这个资源文件。
 * </p>
 *
 * <p>
 * 注意，<code>Thread.getContextClassLoader()</code>是在JDK1.2之后才有的，对于低版本的JDK， <code>ClassLoaderUtil</code>的效果和直接调用
 * <code>ClassLoader</code>完全相同。
 * </p>
 *
...
public abstract class ClassLoaderUtil {
     ...
     * @throws ClassNotFoundException 如果service class找不到，或装载service文件失败
     */
    public static <T> Class<T> loadServiceClass(String serviceId, ClassLoader classLoader)
        ...
        try {
            serviceClassName = StringUtil.trimToEmpty(StreamUtil.readText(istream, "UTF-8", true));
        } catch (IOException e) {
    ...
    /**
     * 从指定的<code>ClassLoader</code>中装入类并实例化之。如果未指定<code>ClassLoader</code>， 则从装载 <code>ClassLoaderUtil</code>的
     * <code>ClassLoader</code>中装入。
     *
     * @param className 要实例化的类名
     * @param classLoader 从指定的<code>ClassLoader</code>中装入类，如果为<code>null</code>，表示从 <code>ClassLoaderUtil</code>所在的class
